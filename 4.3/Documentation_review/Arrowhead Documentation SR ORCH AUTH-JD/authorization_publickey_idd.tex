...
\ArrowheadTitle{Authorization Publickey HTTP/TLS/JSON}
...
The rest of this document describes how to realize the Auhtorization Public Key service using HTTP \cite{fielding2014hypertext}, TLS \cite{rescorla2018transport} and JSON \cite{bray2014json}, both in terms of its functions (Section \ref{sec:functions}) and its information model (Section \ref{sec:model}).
...
This section lists the functions that must be exposed by the  Auhtorization Public Key service in alphabetical order.
In particular, each subsection first names the HTTP method and path used to call the function, after which it names an abstract function from the  Auhtorization Public Key SD document, as well as input and output types.
...
\fsubsection{Publickey}{GET}{/authorization/publickey}{Publickey}{}

Called to get the public key of the Authorization Core System, as exemplified in Listing \ref{lst:query}.

\begin{lstlisting}[language=http,label={lst:query},caption={A \fref{Publickey} invocation.}]
GET /authorization/publickey HTTP/1.1
\end{lstlisting}

Response of the call above:

\begin{lstlisting}[language=http,label={lst:query},caption={A \fref{Publickey} response}]

\end{lstlisting}
\newpage
